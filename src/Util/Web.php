...
final class Web
{
     ...
     * @return array
     */
    public function addDiacritics(array $diacritics): Web
    {
        foreach ($diacritics as $key => $value) {
            $this->diacritics[$key] = $value;
        }
     ...
     * @return int
     */
    public function send(string $file, string $name = null, string $mime = null, int $kbps = 0, bool $force = true): int
    {
        if (!is_file($file)) {
            return -1;
        }
        ...
        $headers['X-Powered-By'] = $this->fw['PACKAGE'];
     ...
     * @param callable|null $slug
     * @param bool          $test
     *
     * @return array
     */
    public function receive(string $dir, bool $overwrite = false, callable $handler = null, callable $slug = null, bool $test = false): array
    {
        $this->fw->mkdir($dir);

        if ('PUT' === $this->fw['VERB']) {
            $tmpDir = $this->fw['TEMP'].'uploads/';
            $tmp = $tmpDir.$this->fw['SEED'].'.'.$this->fw->hash(uniqid());

            $this->fw->mkdir($tmpDir);
            $saved = false !== ($this->fw['RAW'] ? $this->writeRaw($tmp) : $this->fw->write($tmp, $this->fw['BODY']));

            $base = basename($this->fw['URI']);
            $filename = $base;

            if (preg_match('/(.+?)(\.\w+)?$/', $base, $parts)) {
                $filename = $slug ? $slug($base) : $this->slug($parts[1]).($parts[2] ?? null);
            }

            $file = $dir.$filename;
            $unwritten = !file_exists($file) || $overwrite;
            $written = false;

            if ($saved && $unwritten) {
                $written = true;
                $unhandled = !$handler || false === $handler(array(
                    'name' => $file,
                    'type' => $this->mime($base),
                    'size' => filesize($tmp),
                    'tmp_name' => $tmp,
        ...
        foreach ($_FILES as $ndx => $item) {
            ...
            foreach (array_filter($names) as $key => $name) {
                $base = basename($name);
                $filename = $base;

                if (preg_match('/(.+?)(\.\w+)?$/', $base, $parts)) {
                    $filename = $slug ? $slug($base, $ndx) : $this->slug($parts[1]).($parts[2] ?? null);
                }
                ...
                if ($unwritten) {
                    $written = true;
                    $unhandled = !$handler || false === $handler($file, $ndx);

                    if ($unhandled) {
                        $call = $test ? 'rename' : 'move_uploaded_file';
                        $written = $call($file['tmp_name'], $file['name']);
                    }
                }
     ...
     * @codeCoverageIgnore
     */
    private function writeRaw(string $file)
    {
