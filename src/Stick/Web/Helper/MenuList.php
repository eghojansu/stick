...
class MenuList
{
    ...
    public function build(array $items, string $activeRoute = null, array $config = null): string
    {
        ...
        foreach ($items as $label => $item) {
            ...
            if (empty($aAttr['href'])) {
                $aAttr['href'] = $mItem['route'] ? $this->urlGenerator->generate($mItem['route'], $mItem['args']) : '#';
            }

            $iAttr['class'] = trim(($iAttr['class'] ?? null).' '.$aClass[$active]) ?: null;

            $content = Util::tag('a', $aAttr, true, $label);
            $lists .= Util::tag('li', $iAttr, true, $content.$child);
        }

        return Util::tag('ul', $config['root_attr'], true, $lists);
    }
}
